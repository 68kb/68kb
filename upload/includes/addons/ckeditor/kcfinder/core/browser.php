...
class browser extends uploader {
    ...
    public function __construct() {
        ...
        $files = glob("{$this->config['uploadDir']}/*.zip");
    ...
    protected function act_newDir() {
        if (!isset($this->post['dir']) || !isset($this->post['newDir']))
            $this->errorMsg("Unknown error.");
        $dir = $this->postDir();
        $newDir = text::clearWhitespaces($this->post['newDir']);
        if (!strlen($newDir))
    ...
    protected function act_renameDir() {
        if (!isset($this->post['dir']) || !isset($this->post['newName']))
            $this->errorMsg("Unknown error.");
        $dir = $this->postDir();
    ...
    protected function act_deleteDir() {
        $dir = $this->postDir();
        if (!isset($this->post['dir']) || !strlen(trim($this->post['dir'])))
            $this->errorMsg("Unknown error.");
    ...
    protected function xmlTree(array $tree) {
        $xml = '<dir readable="' . ($tree['readable'] ? "yes" : "no") . '" writable="' . ($tree['writable'] ? "yes" : "no") . '" removable="' . ($tree['removable'] ? "yes" : "no") . '" hasDirs="' . ($tree['hasDirs'] ? "yes" : "no") . '"' . (isset($tree['current']) ? ' current="yes"' : '') . '><name><![CDATA[' . $tree['name'] . ']]></name>';
